...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (choiceNumber==0) {
        ...
        NSString* type=[NSString stringWithFormat:@"%@",[dict objectForKey:@"p_id"]];
        NSLog(@"type:%@",type);
        cell.backgroundView=[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"messagefriends@2x.png"]];
        //申请添加好友的消息
        if ([type isEqualToString:@"0"])
        {
            //NSLog(@"申请添加好友");
    ...
    if (choiceNumber==1) {
        ...
        if([[self.partyId objectForKey:@"P_UUID"] isEqualToNumber:self.uuid]){//如果派对的创建者的id等于uuid
            if([[self.user objectForKey:@"USER_ID"] isEqualToNumber:self.uuid]){//如果信息接收者的id等于uuid
                //***************************************图片后面的背景*********************************************
                cell.backgroundView=[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"MASSAGEjoinup@2x.png"]];
                //***************************************图片后面的背景  end*********************************************
                ...
                if ([[[[self.message objectAtIndex:indexPath.section] objectForKey:@"M_STATUS"]substringToIndex:1] isEqualToString:@"W"]) {
                    ...
                    UIButton *button1=[UIButton buttonWithType:UIButtonTypeCustom];
                    button1.frame=CGRectMake(192, 110, 54, 21);
                    button1.backgroundColor=[UIColor clearColor];
                    ...
                    UIButton *button1=[UIButton buttonWithType:UIButtonTypeCustom];
                    button1.frame=CGRectMake(27, 112, 58, 15);
                    button1.backgroundColor=[UIColor clearColor];
                ...
                //***************************************图片后面的背景*********************************************
                cell.backgroundView=[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"MASSAGEME@2x.png"]];
                //***************************************图片后面的背景  end*********************************************
                ...
                if ([[[[self.message objectAtIndex:indexPath.section] objectForKey:@"M_STATUS"]substringToIndex:1] isEqualToString:@"W"]) {
                    ...
                    UIButton *button1=[UIButton buttonWithType:UIButtonTypeCustom];
                    button1.Frame=CGRectMake(27, 110, 54, 21);
                    button1.backgroundColor=[UIColor clearColor];
            ...
            if(isdone){
                ...
                if ([[[[self.message objectAtIndex:indexPath.section] objectForKey:@"M_STATUS"]substringToIndex:1] isEqualToString:@"W"]) {
                    ...
                    UIButton *button1=[UIButton buttonWithType:UIButtonTypeCustom];
                    button1.frame=CGRectMake(27, 112, 54, 21);
                    button1.backgroundColor=[UIColor clearColor];
                    [button1 setImage:[UIImage imageNamed:@"YCJ@2x.png"] forState:UIControlStateNormal];
                    [cell.contentView addSubview:button1];
    ...
    if (choiceNumber==2) {
        ...
        if ([stringP_ID intValue] !=0) {
            //排队入场券
            cell.backgroundView=[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"PRtick@2x.png"]];
        ...
        {
            cell.backgroundView=[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"ACTytick@2x.png"]];
